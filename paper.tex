...
While this paper does not present new experimental results—its focus is the software system itself—all of these prior studies have been made fully reproducible using the example pipelines included in the \texttt{examples} directory of the source repository (\url{https://github.com/simplymathematics/deckard}
). 
One such published work, now reproducible via \texttt{examples/attack_defence_survey}, provides a large survey of attacks and defenses against canonical datasets and models (C. Meyers, Löfstedt, and Elmroth 2023). 
...
Another work formalized a method for estimating a model’s time-to-failure under a suite of attacks and introduced a metric comparing attack cost to training cost (reproducible via \texttt{examples/survival_heuristic}) (Meyers et al. 2023). 
