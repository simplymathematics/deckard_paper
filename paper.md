...
<!-- TODO: provide link to exaples documentation. -->
...
Due to the large scale and cost associated with training popular models (_e.g._, neural networks), it is often necessary to tune a model using hundreds or thousands of indivudal model configurations (often called _hyper-paremeters_).
In addition, even more simple models are often part of various long and complex data and software pipelines.
In short, the typical ML model has a  very large number of hyper-parameters that must be configured, managed, and evaluated.
